...
export default function EditDraftForm({
  ...
  useEffect(() => {
    ...
    states.forEach((part, idx) => {
      ...
      Promise.all([
        getDraftPartPredictionRules(accessToken, part.partPublicId),
        getDraftPartPredictors(accessToken, part.partPublicId),
        ...
        setPartStates((prev) =>
          prev.map((p, i) =>
            i === idx ? { ...p, predictionConfig: config, predictionsLoaded: true } : p
          )
        );
      });
    });
  ...
  function updatePredictionConfigOnPart(partIdx: number, config: PredictionConfig) {
    setPartStates((prev) =>
      prev.map((p, i) => (i !== partIdx ? p : { ...p, predictionConfig: config }))
    );
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Step 3b: Episode number — only meaningful once the draft has completed.
      // Upsert only; there's no clear/unset endpoint, so a blank field is a no-op
      // rather than an attempt to remove an existing number.
      if (draft.draftStatus.name === "Completed") {
      ...
      if (draft.draftStatus.name !== "Completed" && draft.draftStatus.name !== "Cancelled") {
        for (const part of partStates) {
          ...
          // Predictions
          if (part.predictionsLoaded) {
            await syncPredictionConfig(accessToken, partPublicId, part.predictionConfig);
          }
        }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-10">
      ...
      {/* ── Episode Number — only once a draft has actually completed ── */}
      {draft.draftStatus.name === "Completed" && (
        <section>
