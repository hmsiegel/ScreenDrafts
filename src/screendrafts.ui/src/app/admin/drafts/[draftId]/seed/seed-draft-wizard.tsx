...
type StepKey = "episode" | "predictions" | "start" | "trivia" | "picks" | "complete";
...
function deriveInitialStepIndex(detail: AdminDraftDetail, part: PartSummary): number {
  if (detail.episodeNumber == null) return 0; // episode
  if (part.status.name === "InProgress") return 3; // trivia — self-hydrates and is skippable,
  // so landing here is a fast click-through if it's already done, but won't
  // silently skip it if it isn't.
  if (part.status.name === "Completed") return 5; // complete
  return 1; // Created, episode already set -> predictions
}
...
export function SeedDraftWizard({ detail, accessToken }: Props) {
  ...
  const [stepIndex, setStepIndex] = useState(() =>
    part ? deriveInitialStepIndex(detail, part) : 0
  );
