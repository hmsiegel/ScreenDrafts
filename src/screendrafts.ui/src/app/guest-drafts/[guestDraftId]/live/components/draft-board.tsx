// app/guest-drafts/[guestDraftId]/live/components/draft-board.tsx
'use client';
...
export function DraftBoard({ activeSlot, renderActions }: DraftBoardProps) {
  const { picks, draftPositions } = useGuestDraftLive();

  const { minPosition, maxPosition } = resolveRange(draftPositions);
...
function resolveRange(draftPositions: ReturnType<typeof useGuestDraftLive>['draftPositions']) {
  const allSlots = draftPositions.flatMap((p) => p.ownedBoardSlots ?? []);
  if (allSlots.length === 0) return { minPosition: 1, maxPosition: 7 };
  return {
    minPosition: Math.min(...allSlots),
    maxPosition: Math.max(...allSlots),
  };
}