...
{
    private readonly IMultiTenantStore<ScreenDraftsTenantInfo> _tenantStore;
    ...
    public TenantService(
        IMultiTenantStore<ScreenDraftsTenantInfo> tenantStore,
    ...
    public async Task<string> CreateAsync(CreateTenantRequest request, CancellationToken cancellationToken)
    {
        ...
        var tenant = new ScreenDraftsTenantInfo(request.Id, request.Name, request.ConnectionString, request.AdminEmail, request.Issuer);
    ...
    private async Task<ScreenDraftsTenantInfo> GetTenantInfoAsync(string id) =>
        await _tenantStore.TryGetAsync(id)
            ?? throw new NotFoundException(_t["{0} {1} Not Found.", typeof(ScreenDraftsTenantInfo).Name, id]);
}