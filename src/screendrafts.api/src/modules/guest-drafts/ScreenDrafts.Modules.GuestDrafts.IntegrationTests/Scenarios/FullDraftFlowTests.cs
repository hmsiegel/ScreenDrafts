  ...
  : GuestDraftsIntegrationTest(factory)
{
  ...
  public async Task FullFlow_WithAFixedStandardLayoutAndTwoParticipants_ShouldReachCompletedAsync()
  {
    ...
    var guestDraftPublicId = await CreateGuestDraftAsync(
      a.UserPublicId,
      DraftType.Standard,
      "Weekend Standard Draft"
    );
    (await AddParticipantAsync(guestDraftPublicId, a.UserPublicId, a.GuestDrafterPublicId))
      .IsSuccess.Should()
      .BeTrue();
    (await AddParticipantAsync(guestDraftPublicId, a.UserPublicId, b.GuestDrafterPublicId))
      .IsSuccess.Should()
      .BeTrue();
    ...
    (await PlayPickAsync(guestDraftPublicId, a.UserPublicId, await CreateMovieAsync(), 7, 1))
      .IsSuccess.Should()
      .BeTrue();
    ...
    (await PlayPickAsync(guestDraftPublicId, a.UserPublicId, await CreateMovieAsync(), 6, 2))
      .IsSuccess.Should()
      .BeTrue();
    (await ApplyCommissionerOverrideAsync(guestDraftPublicId, 2, a.UserPublicId))
      .IsSuccess.Should()
      .BeTrue();

    // 7. Undo the veto -- restores slot 7 to landed
    (await UndoVetoAsync(guestDraftPublicId, 1, a.UserPublicId))
      .IsSuccess.Should()
      .BeTrue();
    ...
    (await UndoPickAsync(guestDraftPublicId, 2, a.UserPublicId))
      .IsSuccess.Should()
      .BeTrue();
    ...
    (await RevealPickAsync(guestDraftPublicId, 1, b.UserPublicId))
      .IsSuccess.Should()
      .BeTrue();

    // 10. Land every remaining slot (6, 4, 2, 5, 3, 1) so the board can complete
    (await PlayPickAsync(guestDraftPublicId, a.UserPublicId, await CreateMovieAsync(), 6, 3))
      .IsSuccess.Should()
      .BeTrue();
    (await PlayPickAsync(guestDraftPublicId, a.UserPublicId, await CreateMovieAsync(), 4, 4))
      .IsSuccess.Should()
      .BeTrue();
    (await PlayPickAsync(guestDraftPublicId, a.UserPublicId, await CreateMovieAsync(), 2, 5))
      .IsSuccess.Should()
      .BeTrue();
    (await PlayPickAsync(guestDraftPublicId, a.UserPublicId, await CreateMovieAsync(), 5, 6))
      .IsSuccess.Should()
      .BeTrue();
    (await PlayPickAsync(guestDraftPublicId, a.UserPublicId, await CreateMovieAsync(), 3, 7))
      .IsSuccess.Should()
      .BeTrue();
    (await PlayPickAsync(guestDraftPublicId, a.UserPublicId, await CreateMovieAsync(), 1, 8))
      .IsSuccess.Should()
      .BeTrue();
    ...
    completeResult.Value.Status.Should().Be(DraftStatus.Completed.Name);
  }
  ...
  public async Task FullFlow_WithACustomLayoutAndThreeParticipants_ShouldReachCompletedAsync()
  {
    ...
    (await AddParticipantAsync(guestDraftPublicId, a.UserPublicId, b.GuestDrafterPublicId))
      .IsSuccess.Should()
      .BeTrue();
    (await AddParticipantAsync(guestDraftPublicId, a.UserPublicId, c.GuestDrafterPublicId))
      .IsSuccess.Should()
      .BeTrue();
    ...
    (
      await AssignParticipantAsync(
        guestDraftPublicId,
        a.UserPublicId,
        boardPositions.Single(p => p.Name == "Pos1").PublicId,
        a.GuestDrafterPublicId
      )
    )
      .IsSuccess.Should()
    ...
    (
      await AssignParticipantAsync(
        guestDraftPublicId,
        a.UserPublicId,
        boardPositions.Single(p => p.Name == "Pos2").PublicId,
        b.GuestDrafterPublicId
      )
    )
      .IsSuccess.Should()
    ...
    (
      await AssignParticipantAsync(
        guestDraftPublicId,
        a.UserPublicId,
        boardPositions.Single(p => p.Name == "Pos3").PublicId,
    ...
    // 5. Play a pick, veto it
    (await PlayPickAsync(guestDraftPublicId, a.UserPublicId, await CreateMovieAsync(), 1, 1))
      .IsSuccess.Should()
      .BeTrue();
    (await ApplyVetoAsync(guestDraftPublicId, 1, b.UserPublicId)).IsSuccess.Should().BeTrue();

    // 6. Play another pick, apply a commissioner override to it
    (await PlayPickAsync(guestDraftPublicId, a.UserPublicId, await CreateMovieAsync(), 2, 2))
      .IsSuccess.Should()
      .BeTrue();
    (await ApplyCommissionerOverrideAsync(guestDraftPublicId, 2, a.UserPublicId))
      .IsSuccess.Should()
    ...
    var revealerParticipant = pickAfterUndo.Participants.Single(p =>
      p.Id == survivingPick.RevealAuthorizedParticipantId
    );
    var revealerUserPublicId =
      revealerParticipant.ParticipantIdValue == b.GuestDrafterId ? b.UserPublicId : c.UserPublicId;
    ...
    // 10. Land slot 2
    (await PlayPickAsync(guestDraftPublicId, a.UserPublicId, await CreateMovieAsync(), 2, 3))
      .IsSuccess.Should()
      .BeTrue();
    ...
    (await PlayPickAsync(guestDraftPublicId, a.UserPublicId, await CreateMovieAsync(), 3, 4))
      .IsSuccess.Should()
      .BeTrue();
    (await ApplyVetoAsync(guestDraftPublicId, 4, c.UserPublicId)).IsSuccess.Should().BeTrue();
    (await ApplyVetoOverrideAsync(guestDraftPublicId, 4, b.UserPublicId))
      .IsSuccess.Should()
      .BeTrue();
