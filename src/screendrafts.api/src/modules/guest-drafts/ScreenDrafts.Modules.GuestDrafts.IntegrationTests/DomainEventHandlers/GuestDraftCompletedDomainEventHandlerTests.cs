...
public sealed class GuestDraftCompletedDomainEventHandlerTests(
...
)
{
  ...
  {
    // Arrange
    var domainEvent = new DraftCompletedDomainEvent(
      draftId: Guid.NewGuid(),
      draftPublicId: $"gd_{TestFakerProvider.Faker.Random.AlphaNumeric(15)}",
      totalPicks: 7,
      vetoCount: 2
    );
    var fixedUtcNow = new DateTime(2026, 1, 1, 12, 0, 0, DateTimeKind.Utc);
    var eventBus = new CapturingEventBus();
    var handler = new DraftCompletedDomainEventHandler(
      eventBus,
    ...
    var published = eventBus
      ...
      .Should()
      .BeOfType<GuestDraftCompletedIntegrationEvent>()
      .Subject;

    published.Id.Should().NotBe(Guid.Empty);
    published.OccurredOnUtc.Should().Be(fixedUtcNow);
    published.GuestDraftId.Should().Be(domainEvent.DraftId);
    published.GuestDraftPublicId.Should().Be(domainEvent.DraftPublicId);
  ...
  public void Handler_ShouldBeDiscoverableByGuestDraftsModulesDomainEventHandlerRegistration()
  {
    ...
    var handler = scope.ServiceProvider.GetService(typeof(DraftCompletedDomainEventHandler));

    handler.Should().NotBeNull();
    handler.Should().BeAssignableTo<IDomainEventHandler<DraftCompletedDomainEvent>>();
  }
}
