...
internal sealed class GuestDraftPositionConfiguration : IEntityTypeConfiguration<DraftPosition>
{
  public void Configure(EntityTypeBuilder<DraftPosition> builder)
  {
    ...
    builder
      .Property(p => p.GameBoardId)
      .IsRequired()
      .HasConversion(IdConverters.GuestDraftGameBoardIdConverter);

    builder.Property(p => p.Name).IsRequired().HasMaxLength(DraftPosition.NameMaxLength);
    ...
    builder.Property(p => p.Picks).IsRequired().HasColumnType("integer[]");

    builder.Property(p => p.HasBonusVeto).IsRequired();

    builder.Property(p => p.HasBonusVetoOverride).IsRequired();

    builder.Property(p => p.HasBonusFungibleToken).IsRequired();
