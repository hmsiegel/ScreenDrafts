...
internal sealed class PickConfiguration : IEntityTypeConfiguration<Pick>
{
  public void Configure(EntityTypeBuilder<Pick> builder)
  {
    ...
    builder
      ...
      .ValueGeneratedNever()
      .HasColumnName("id")
      .HasConversion(IdConverters.PickIdConverter);

    builder.Property(p => p.GuestDraftId).IsRequired().HasConversion(IdConverters.DraftIdConverter);
    ...
    builder
      ...
      .HasConversion(IdConverters.NullableDraftParticipantIdConverter);
    ...
    builder.OwnsMany(
      ...
      history =>
      {
        history.ToTable(Tables.PickHistory);
        history.WithOwner().HasForeignKey("PickId");
