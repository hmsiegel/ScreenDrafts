...
internal sealed class GetDraftGameplayQueryHandler(
  IDbConnectionFactory dbConnectionFactory,
  IUsersApi usersApi
) : IQueryHandler<GetDraftGameplayQuery, GetGuestDraftGameplayResponse>
{
  public async Task<Result<GetGuestDraftGameplayResponse>> Handle(
    GetDraftGameplayQuery request,
  ...
  )
  {
    ...
    const string vetoHistorySql = $"""
      ...
      FROM guest_drafts.vetoes v
      JOIN guest_drafts.picks pk ON pk.id = v.target_pick_id
      JOIN guest_drafts.drafts gd ON gd.id = pk.guest_draft_id
      LEFT JOIN guest_drafts.veto_overrides vo ON vo.veto_id = v.id
      WHERE gd.public_id = @GuestDraftPublicId
