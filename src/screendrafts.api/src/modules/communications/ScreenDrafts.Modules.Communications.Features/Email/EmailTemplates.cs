...
internal static class EmailTemplates
{
  ...
  // NEW — steady-state email change confirmation. Same BaseLayout as everything
  // else here; not touching styling per your note that the whole template set
  // is getting a theme pass separately.
  ...
  )
  {
    return BaseLayout(
      ...
      $"""
      <p>You requested to change the email on your ScreenDrafts account to <strong>{Encode(
        newEmail
      )}</strong>.</p>
      <p>
        <a href="{Encode(
        confirmationLink
      )}" style="display:inline-block;background:#1a1a2e;color:#fff;padding:10px 20px;border-radius:4px;text-decoration:none;">
          Confirm email change
        </a>
      </p>
      <p>If you didn't request this, you can ignore this email — your address won't change unless you click the link above.</p>
      """
    );
  }
              ...
              <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;">
                ...
                <tr>
                  <td style="padding:32px;">
                    <p style="margin:0 0 16px;font-size:16px;color:#333;">Hi {Encode(
        recipientName
      )},</p>
