...
internal sealed class DraftRepository(DraftsDbContext dbContext) : IDraftRepository
{
  ...
  {
    var draft = await _dbContext.Drafts.FirstOrDefaultAsync(
      d => d.Id == draftId,
      cancellationToken
    );
  ...
  {
    return await _dbContext.Drafts.FirstOrDefaultAsync(
  ...
  public async Task<Draft?> GetDraftByPublicIdWithPartsAsNoTrackingAsync(
    string publicId,
  ...
  public async Task<Draft?> GetDraftByPublicIdWithPartsAsync(
    string publicId,
    CancellationToken cancellationToken
  )
  {
    return await _dbContext
      ...
      .Include("Parts._picks._vetoes")
      .Include("Parts._picks.CommissionerOverride")
      .Include("Parts._subDrafts")
      .FirstOrDefaultAsync(d => d.PublicId == publicId, cancellationToken);
  }

  public Task<Draft?> GetDraftByPublicIdForUpdateAsync(
    string publicId,
    CancellationToken cancellationToken
  )
  {
  ...
  public Task<Draft?> GetByIdWithPartsAndParticipantsAsync(
    DraftId draftId,
    CancellationToken cancellationToken
  )
  {
