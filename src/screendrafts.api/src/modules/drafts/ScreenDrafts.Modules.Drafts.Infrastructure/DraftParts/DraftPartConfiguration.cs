...
internal sealed class DraftPartConfiguration : IEntityTypeConfiguration<DraftPart>
{
  public void Configure(EntityTypeBuilder<DraftPart> builder)
  {
    ...
    builder.Property(x => x.PartIndex).IsRequired();

    builder.HasIndex(x => new { x.DraftId, x.PartIndex }).IsUnique();
    ...
    builder
      .HasMany<DraftRelease>("_releases")
      .WithOne(r => r.DraftPart)
      .HasForeignKey(r => r.PartId)
      .OnDelete(DeleteBehavior.Cascade);

    builder.Navigation("_releases").UsePropertyAccessMode(PropertyAccessMode.Field);

    // Status
    builder
      .Property(d => d.Status)
      .HasConversion(dps => dps.Value, value => DraftPartStatus.FromValue(value));
    ...
    builder.Navigation("_picks").UsePropertyAccessMode(PropertyAccessMode.Field);

    builder.Property(dp => dp.MovieVersionPolicyType).HasConversion<int>().IsRequired();

    builder.OwnsMany<RequiredMovieVersion>(
      "_requiredMovieVersions",
      b =>
      {
        ...
        b.Property(x => x.MovieId).HasColumnName("movie_id").IsRequired();

        b.Property(x => x.VersionName).HasColumnName("version_name").HasMaxLength(100).IsRequired();

        b.HasIndex("draft_part_id", nameof(RequiredMovieVersion.MovieId)).IsUnique();
      }
    );

    builder.Navigation("_requiredMovieVersions").UsePropertyAccessMode(PropertyAccessMode.Field);
    ...
    builder
      .HasMany<DraftPartParticipant>("_draftPartParticipants")
      .WithOne(dpp => dpp.DraftPart)
      .HasForeignKey(dpp => dpp.DraftPartId)
      .OnDelete(DeleteBehavior.Cascade);

    builder.Navigation("_draftPartParticipants").UsePropertyAccessMode(PropertyAccessMode.Field);

    // Draft Type
    builder
      .Property(d => d.DraftType)
      .IsRequired()
      .HasConversion(dt => dt.Value, value => DraftType.FromValue(value));

    // Hosts
    builder.Ignore(d => d.DraftHosts);

    builder
      .HasMany<DraftHost>("_draftHosts")
    ...
    // Community Limits
    builder.Property(dp => dp.MaxCommunityPicks).HasDefaultValue(0).IsRequired();

    builder.Property(dp => dp.MaxCommunityVetoes).HasDefaultValue(0).IsRequired();

    // ── Community Film Rules ────────────────────────────────────────────────────

    builder.OwnsMany<CommunityFilmRule>(
      "_communityFilmRules",
      b =>
      {
        b.ToTable(Tables.DraftPartCommunityFilmRules);
        b.WithOwner().HasForeignKey("draft_part_id");
    ...
    builder
      .HasMany<SubDraft>("_subDrafts")
      .WithOne()
      .HasForeignKey(sd => sd.DraftPartId)
      .OnDelete(DeleteBehavior.Cascade);

    builder.Navigation("_subDrafts").UsePropertyAccessMode(PropertyAccessMode.Field);

    builder.Property(d => d.SubDraftCount);

    // Zoom
    builder.Property(d => d.ZoomSessionName).HasMaxLength(200);

    builder
      .HasIndex(d => d.ZoomSessionName)
      .IsUnique()
