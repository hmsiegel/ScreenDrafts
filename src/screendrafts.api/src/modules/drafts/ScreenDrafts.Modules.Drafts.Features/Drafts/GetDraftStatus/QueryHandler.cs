  ...
  : IQueryHandler<Query, Response>
{
  ...
  public async Task<Result<Response>> Handle(Query request, CancellationToken cancellationToken)
  {
    ...
    var draft = await _draftsRepository.GetDraftByPublicIdWithPartsAsNoTrackingAsync(request.DraftPublicId, cancellationToken);

    if (draft is null)
    {
      return Result.Failure<Response>(DraftErrors.NotFound(request.DraftPublicId));
    }
