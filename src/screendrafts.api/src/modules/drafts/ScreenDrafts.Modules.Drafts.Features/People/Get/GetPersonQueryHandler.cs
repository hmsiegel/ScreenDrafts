...
internal sealed class GetPersonQueryHandler(IDbConnectionFactory dbConnectionFactory) : IQueryHandler<GetPersonQuery, PersonResponse>
{
  private readonly IDbConnectionFactory _dbConnectionFactory = dbConnectionFactory;

  public async Task<Result<PersonResponse>> Handle(GetPersonQuery GetPersonRequest, CancellationToken cancellationToken)
  {
    ...
    var person = await connection.QuerySingleOrDefaultAsync<PersonResponse>(new CommandDefinition(
      sql,
      new { GetPersonRequest.PublicId },
      cancellationToken: cancellationToken));

    if (person is null)
    {
      return Result.Failure<PersonResponse>(PersonErrors.NotFound(GetPersonRequest.PublicId));
    }
