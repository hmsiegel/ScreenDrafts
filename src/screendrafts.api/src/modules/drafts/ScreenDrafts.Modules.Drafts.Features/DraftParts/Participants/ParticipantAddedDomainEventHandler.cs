...
) : DomainEventHandler<ParticipantAddedDomainEvent>
{
  ...
  )
  {
    ...
    var (newParticipantUserIds, newParticipantName) =
    ...
    var context = await connection.QuerySingleAsync<DraftPartContextRow>(
      new CommandDefinition(
        ...
        parameters: new { domainEvent.ParticipantIdValue, domainEvent.DraftPartId },
    ...
    const string coParticipantsQuery = """
      SELECT
          ...
          CASE dpp.participant_kind_value
              WHEN 0 THEN CONCAT(p.first_name, ' ', p.last_name)
              WHEN 1 THEN dt.name
              ELSE NULL
          END AS Name
    ...
    var coParticipants = (
      await connection.QueryAsync<CoParticipantRow>(
        new CommandDefinition(
          commandText: coParticipantsQuery,
          parameters: new { domainEvent.DraftPartId, domainEvent.ParticipantIdValue },
          cancellationToken: cancellationToken
        )
      )
    )
  ...
  )
  {
    ...
    var userIds = result.UserId.HasValue ? (IReadOnlyList<Guid>)[result.UserId.Value] : [];
