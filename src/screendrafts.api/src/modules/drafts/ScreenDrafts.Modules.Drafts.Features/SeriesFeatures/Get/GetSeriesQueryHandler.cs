...
internal sealed partial class GetSeriesQueryHandler(IDbConnectionFactory dbConnectionFactory) : IQueryHandler<GetSeriesQuery, SeriesResponse>
{
  private readonly IDbConnectionFactory _dbConnectionFactory = dbConnectionFactory;

  public async Task<Result<SeriesResponse>> Handle(GetSeriesQuery GetSeriesFeatureRequest, CancellationToken cancellationToken)
  {
    ...
    var row = await connection.QuerySingleOrDefaultAsync<SeriesRow>(new CommandDefinition(
      sql,
      new { GetSeriesFeatureRequest.PublicId },
      cancellationToken: cancellationToken));

    return row is not null
      ? Result.Success(QueryMapping.Map(row))
      : Result.Failure<SeriesResponse>(CampaignErrors.NotFound(GetSeriesFeatureRequest.PublicId));
  }
}
