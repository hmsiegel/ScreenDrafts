...
public static class PredictionFactory
{
  ...
  public static DraftPartPredictionRule CreateUnorderedAllRule(
    DraftPart draftPart,
    int requiredCount = 3
  ) =>
    DraftPartPredictionRule
      .Create(
        publicId: _faker.Random.AlphaNumeric(10),
        draftPart: draftPart,
  ...
  public static DraftPartPredictionRule CreateUnorderedTopNRule(
    DraftPart draftPart,
    int requiredCount = 3,
    int topN = 5
  ) =>
  ...
  )
  {
    ...
    var entries = ids.Select((id, idx) => PredictionEntry.Create(set, id, $"Movie {idx + 1}"))
      .ToList();
