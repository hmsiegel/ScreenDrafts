...
{
  public static readonly SDError RolloverVetoAlreadyExists = SDError.Conflict(
    "Drafters.RolloverVetoAlreadyExists",
    "A rollover veto already exists for this drafter."
  );

  public static readonly SDError RolloverVetoOverrideAlreadyExists = SDError.Conflict(
  ...
  public static SDError NotFound(Guid drafterId) =>
    SDError.NotFound("Drafters.NotFound", $"Unable to find a drafter with Id {drafterId}.");

  public static SDError NotFound(string publicId) =>
    SDError.NotFound("Drafters.NotFound", $"Unable to find a drafter with Id {publicId}.");

  public static SDError VetoNotFound(Guid vetoId) =>
    SDError.NotFound("Drafters.VetoNotFound", $"Veto with ID {vetoId} not found.");

  public static readonly SDError CannotCreatDrafter = SDError.Failure(
    "Drafters.CannotCreatDrafter",
    "Cannot create drafter without user ID or name."
  );

  public static readonly SDError InvalidQuestionsWon = SDError.Failure(
    "Drafters.InvalidQuestionsWon",
    "Questions won must be greater than or equal to 0."
  );

  public static readonly SDError InvalidPosition = SDError.Failure(
  ...
  public static SDError NotFoundForPerson(string publicId) =>
    SDError.NotFound(
      "Drafters.NotFoundForPerson",
      $"Unable to find a drafter for person with public ID {publicId}."
    );

  public static readonly SDError NotEnoughDrafters = SDError.Failure(
    "Drafters.NotEnoughDrafters",
    "There must be at least two drafters on a team."
  );

  public static readonly SDError InvalidBlessingRequest = SDError.Failure(
    "Drafters.InvalidBlessingRequest",
    "Invalid blessing request. You must specity either a drafter or a drafter team, but not both."
  );

  public static readonly SDError AlreadyRetired = SDError.Conflict(
