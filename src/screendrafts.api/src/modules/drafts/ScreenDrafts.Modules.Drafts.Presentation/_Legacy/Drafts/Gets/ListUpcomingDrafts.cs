...
using ScreenDrafts.Modules.Drafts.Application._Legacy.Drafts.Queries.ListUpcomingDrafts;
...
internal sealed class ListUpcomingDrafts(ISender sender, IUsersApi usersApi) : EndpointWithoutRequest<List<UpcomingDraftResponse>>
{
  ...
  public override async Task HandleAsync(CancellationToken ct)
  {
    ...
    var isAdmin = userRoles.Contains(_Legacy.Roles.Admin, StringComparer.OrdinalIgnoreCase) || userRoles.Contains(_Legacy.Roles.SuperAdmin, StringComparer.OrdinalIgnoreCase);
    var canViewPatreon = User.HasClaim(p => p.Type == "permission" && p.Value == _Legacy.Permissions.PatronSearchDrafts);
