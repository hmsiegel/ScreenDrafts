  ...
  : DraftsIntegrationTest(factory)
{
  ...
  )> SetupActiveSubDraftAsync()
  {
    ...
    var subDraftPublicId = (
      await Sender.Send(
        new AddSubDraftCommand { DraftPartPublicId = draftPartPublicId, Index = 0 },
        TestContext.Current.CancellationToken
      )
    ).Value;
    ...
    await Sender.Send(
      new AssignSubDraftTriviaCommand
      {
        DraftPartPublicId = draftPartPublicId,
        SubDraftPublicId = subDraftPublicId,
        Results =
  ...
  )> SetupTwoSubDraftsWithFirstActiveAsync()
  {
    ...
    await Sender.Send(
      new AddSubDraftCommand { DraftPartPublicId = draftPartPublicId, Index = 1 },
      TestContext.Current.CancellationToken
    );
