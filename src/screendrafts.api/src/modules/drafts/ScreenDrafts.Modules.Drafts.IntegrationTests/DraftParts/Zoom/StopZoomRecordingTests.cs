...
public sealed class StopZoomRecordingTests : DraftsIntegrationTest
{
  private readonly DraftsIntegrationTestWebAppFactory _factory;

  public StopZoomRecordingTests(DraftsIntegrationTestWebAppFactory factory) : base(factory)
    => _factory = factory;
  // -------------------------------------------------------------------------
  // Happy path
  // -------------------------------------------------------------------------
  ...
  [Fact]
  public async Task StopZoomRecording_ShouldPublishIntegrationEvent_WhenSessionIsActiveAsync()
  {
    // Arrange
    var (draftPartPublicId, sessionName) = await SetupDraftPartWithSessionAsync();
    _factory.EventBusCapture.Clear();

    var command = new StopZoomRecordingCommand { DraftPartPublicId = draftPartPublicId };
    ...
    var published = _factory.EventBusCapture.CapturedEvents
      .OfType<StopZoomRecordingRequestedIntegrationEvent>()
      .ToList();
  ...
  [Fact]
  public async Task StopZoomRecording_ShouldFail_WhenDraftPartNotFoundAsync()
  {
    // Arrange
  ...
  [Fact]
  public async Task StopZoomRecording_ShouldFail_WhenNoActiveSessionAsync()
  {
