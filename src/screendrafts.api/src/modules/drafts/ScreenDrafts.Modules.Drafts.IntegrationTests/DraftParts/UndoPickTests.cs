  ...
  : DraftsIntegrationTest(factory)
{
  ...
  public async Task UndoPick_WithValidData_ShouldSucceedAsync()
  {
    ...
    var command = new UndoPickCommand { DraftPartId = draftPartPublicId, PlayOrder = 1 };
  ...
  public async Task UndoPick_ShouldRemovePickFromDatabaseAsync()
  {
    ...
    var pickExists = await DbContext.Picks.AnyAsync(
      p => p.PlayOrder == 1 && p.DraftPart.PublicId == draftPartPublicId,
      TestContext.Current.CancellationToken
    );
    ...
    // Act
    await Sender.Send(
      new UndoPickCommand { DraftPartId = draftPartPublicId, PlayOrder = 1 },
      TestContext.Current.CancellationToken
    );

    // Assert — pick is gone
    var pickAfterUndo = await DbContext.Picks.AnyAsync(
      p => p.PlayOrder == 1 && p.DraftPart.PublicId == draftPartPublicId,
      TestContext.Current.CancellationToken
    );
  ...
  public async Task UndoPick_ShouldOnlyRemoveTargetPickAsync()
  {
    ...
    var (draftPartPublicId, drafter1PublicId, drafter2PublicId) =
      await SetupStartedDraftPartAsync();
    ...
    var pick1Exists = await DbContext.Picks.AnyAsync(
      p => p.PlayOrder == 1 && p.DraftPart.PublicId == draftPartPublicId,
      TestContext.Current.CancellationToken
    );
    var pick2Exists = await DbContext.Picks.AnyAsync(
      p => p.PlayOrder == 2 && p.DraftPart.PublicId == draftPartPublicId,
      TestContext.Current.CancellationToken
    );
  ...
  public async Task UndoPick_WithNonExistentPlayOrder_ShouldFailAsync()
  {
    ...
    var command = new UndoPickCommand { DraftPartId = draftPartPublicId, PlayOrder = 99 };
  ...
  public async Task UndoPick_WhenPickIsCommissionerOverridden_ShouldFailAsync()
  {
    ...
    await Sender.Send(
      new ApplyCommissionerOverrideCommand { DraftPartId = draftPartPublicId, PlayOrder = 1 },
      TestContext.Current.CancellationToken
    );
  ...
  private async Task<(
    ...
    string Drafter1PublicId,
    string Drafter2PublicId
  )> SetupStartedDraftPartAsync()
  {
    ...
    var hostPublicId = (
      await Sender.Send(
        new CreateHostCommand { PersonPublicId = hostPersonId },
        TestContext.Current.CancellationToken
      )
    ).Value;
    await Sender.Send(
  ...
  private async Task<Movie> CreateMovieAsync()
  {
    var movie = Movie
      .Create(
        ...
        $"m_{Faker.Random.AlphaNumeric(21)}",
        MediaType.Movie,
        Guid.NewGuid()
      )
      .Value;
