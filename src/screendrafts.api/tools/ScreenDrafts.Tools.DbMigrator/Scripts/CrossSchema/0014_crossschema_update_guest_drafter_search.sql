...
WHERE rp.permission_code IN ('guest-drafters:search')
ON CONFLICT DO NOTHING;

DELETE FROM administration.role_permissions
WHERE permission_code = 'guest-drafter:search'
  AND role_name = 'Guest';

DELETE FROM administration.permissions
WHERE code = 'guest-drafter:search';
