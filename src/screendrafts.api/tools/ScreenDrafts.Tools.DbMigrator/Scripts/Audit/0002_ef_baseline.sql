...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM audit."__EFMigrationsHistory" WHERE "migration_id" = '20250203034826_Add_InboxAndOutbox') THEN
        IF NOT EXISTS(SELECT 1 FROM pg_namespace WHERE nspname = 'audit') THEN
            CREATE SCHEMA audit;
        END IF;
    END IF;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM audit."__EFMigrationsHistory" WHERE "migration_id" = '20250203034826_Add_InboxAndOutbox') THEN
    CREATE TABLE audit.inbox_message_consumers (
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM audit."__EFMigrationsHistory" WHERE "migration_id" = '20250203034826_Add_InboxAndOutbox') THEN
    CREATE TABLE audit.outbox_message_consumers (
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM audit."__EFMigrationsHistory" WHERE "migration_id" = '20250203034826_Add_InboxAndOutbox') THEN
    INSERT INTO audit."__EFMigrationsHistory" (migration_id, product_version)
    VALUES ('20250203034826_Add_InboxAndOutbox', '10.0.3');
    END IF;
END $EF$;
COMMIT;
